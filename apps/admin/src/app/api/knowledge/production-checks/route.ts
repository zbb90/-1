...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const row = await appendRow(TABLE, body as Record<string, string>);
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const updated = await updateRow(TABLE, body.id, body.data);
    if (!updated)
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const updated = await patchRowStatus(TABLE, body.id, body.status);
    if (!updated)
      return NextResponse.json({ ok: false, message: "未找到条目。" }, { status: 404 });
    return NextResponse.json({ ok: true, data: updated });
  } catch (error) {
