...
import { upsertFaqVectors } from "@/lib/vector-store";
import type { FaqRow } from "@/lib/types";

async function syncFaqVector(row: unknown, op: string) {
  try {
    const result = await upsertFaqVectors([row as FaqRow]);
    if (!result.ok) {
      console.warn(`[faq][${op}] vector sync skipped`, result.reason);
    }
  } catch (error) {
    console.warn(`[faq][${op}] vector sync failed`, error);
  }
}

export async function GET() {
  try {
    const rows = await readRows("faq");
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const row = await appendRow("faq", body as Record<string, string>);
    await syncFaqVector(row, "POST");
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const updated = await updateRow("faq", body.id, body.data);
    if (!updated)
      return NextResponse.json({ ok: false, message: "未找到条目。" }, { status: 404 });
    await syncFaqVector(updated, "PUT");
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const updated = await patchRowStatus("faq", body.id, body.status);
    if (!updated)
      return NextResponse.json({ ok: false, message: "未找到条目。" }, { status: 404 });
    await syncFaqVector(updated, "PATCH");
    return NextResponse.json({ ok: true, data: updated });
  } catch (error) {
    return NextResponse.json(
      { ok: false, message: error instanceof Error ? error.message : "更新失败" },
